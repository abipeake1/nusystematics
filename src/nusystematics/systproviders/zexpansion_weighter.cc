...
Eigen::MatrixXd GetPCAFromCovarianceMatrix(Eigen::MatrixXd const &Covariance_Matrix){
 ...
 return eigenvector_colmatrix;
  ...
  Eigen::VectorXd ChangeBasisBParams(std::vector<double> const &BVals, Eigen::MatrixXd const &PCA){
...
std::array<std::string, 4> b_params_for_zexp_names = {"b1", "b2", "b3", "b4"}; //the  b params to be rotated using PCA (in fcl file)
...
//  flag values
ReWeighter::ReWeighter(fhicl::ParameterSet const &params) //fn takes the parameters in fcl file
...
SystMetaData ReWeighter::BuildSystMetaData(fhicl::ParameterSet const &ps,
  ...
  tool_options.put("verbosity_level", ps.get<int>("verbosity_level", 0)); //put tool config options in papam geaders
...
bool ReWeighter::SetupResponseCalculator(
  ...
  // loop through the named parameters (i) that this tool provides, check that they
  ...
  for (size_t i = 0; i < b_params_for_zexp_names.size(); ++i) { 
    ...
    if (!HasParam(md, b_params_for_zexp_names[i])) {  //do we have the b params in the header file?
      if (verbosity_level > 1) {
        std::cout << "[INFO]: Don't have parameter " << b_params_for_zexp_names[i]
                  << " in SystMetaData. Skipping configuration." << std::endl;
      }
      continue;
    }
    pidx_Params[i] = GetParamIndex(md, b_params_for_zexp_names[i]); //the id of the ith param is the paaram index which is defined by headers+name

    if (verbosity_level > 1) {
      std::cout << "[INFO]: Have parameter " << b_params_for_zexp_names[i] //if params are present, start config steps
                << " in SystMetaData with ParamId: " << pidx_Params[i]
                << ". Configuring." << std::endl;
    }

    auto param_md = md[pidx_Params[i]];  //param_md 
    CVs[i] = param_md.centralParamValue; //the initial CV of parameter i (i=1,2,3,4)
    Variations[i] = param_md.paramVariations;  //the variation of parameter i (i=1,2,3,4)
    ...
    for (auto v : Variations[i]) { //loop through the variations of each param e.g (-3,-2...,2,3)std in fcl file
      ...
      bvariations[i]=v;
      //Get the b parameter variations, using the variations in the parameter md and the rotation function
      Eigen::VectorXd a_variations = ChangeBasisBParams(bvariations, GetPCAFromCovarianceMatrix(PRD_93_113015::Covariance_Matrix));
      ...
      for(size_t aval_j = 0; aval_j < 4; ++aval_j) { 
          std::cout<<"  a[" << aval_j << "] param = " << a_variations[aval_j] << ", scaled for GENIE = " << myaparameters[aval_j] <<std::endl;
          ReWeightEngines_new[i].back()->SetSystematic(zexp_dials[aval_j], myaparameters[aval_j]);
      }
...
ReWeighter::GetEventResponse(genie::EventRecord const &ev) {
  ...
  for (size_t b_i = 0; b_i < b_params_for_zexp_names.size(); ++b_i) {
    ...
    for (size_t v_it = 0; v_it < Variations[b_i].size(); ++v_it) {
      ...
      // response object
      resp.back().responses.push_back(ReWeightEngines_new[b_i][v_it]->CalcWeight(ev));
      
      
      if (verbosity_level > 3) {
        std::cout << "[DEBG]: For parameter " << md[pidx_Params[b_i]].prettyName
                  << "The  central value of the parameter is  " <<    CVs[b_i]
                  << " at variation[" << v_it << "] std = " << Variations[b_i][v_it]
